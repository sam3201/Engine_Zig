...
const GameInstance = struct {
    ...
    fn gameLoop(self: *GameInstance) void {
        std.debug.print("Game instance {} started on dedicated thread\n", .{self.client_id});

        const UpdateFunctions = struct {
            instance: *GameInstance,

            fn update(ctx: *@This()) void {
                ctx.instance.mutex.lock();
                defer ctx.instance.mutex.unlock();

                // Clear canvas
                ctx.instance.engine.canvas.clear();

                // Draw player
                ctx.instance.player.draw(&ctx.instance.engine.canvas);

                // Draw UI
                drawInstanceUI(&ctx.instance.engine, ctx.instance);
            }
        };

        var update_ctx = UpdateFunctions{ .instance = self };
        self.engine.setUpdateFn(&@TypeOf(update_ctx).update, &update_ctx);

        // Run the engine loop
        self.engine.run() catch |err| {
            std.debug.print("Engine error for instance {}: {}\n", .{ self.client_id, err });
        };

        std.debug.print("Game instance {} terminated\n", .{self.client_id});
    }
};
...
const GameServer = struct {
    ...
    pub fn createInstance(self: *GameServer, is_wasd: bool) !*GameInstance {
        ...
        std.debug.print("Created game instance {} ({})\n", .{ client_id, if (is_wasd) "WASD" else "HJKL" });
    ...
    pub fn startServerEngine(self: *GameServer) !void {
        ...
        const UpdateFunctions = struct {
            ...
            fn update(ctx: *@This()) void {
                ...
                // Clear server canvas
                ctx.server.server_engine.canvas.clear();

                // Draw server overview
                drawServerOverview(&ctx.server.server_engine, ctx.server);
            }
        };

        var update_ctx = UpdateFunctions{ .server = self };
        self.server_engine.setUpdateFn(&@TypeOf(update_ctx).update, &update_ctx);
...
fn drawInstanceUI(engine: *Engine.Engine, instance: *GameInstance) void {
    ...
    const thread_info = std.fmt.allocPrint(std.heap.page_allocator, "Client {} Thread", .{instance.client_id}) catch return;
    ...
    const pos_text = std.fmt.allocPrint(std.heap.page_allocator, "Position: ({}, {})", .{ pos.x, pos.y }) catch return;
...
fn drawServerOverview(engine: *Engine.Engine, server: *GameServer) void {
    ...
    const cpu_text = std.fmt.allocPrint(std.heap.page_allocator, "Available CPU Cores: {}", .{cpu_count}) catch return;
    ...
    const instance_text = std.fmt.allocPrint(std.heap.page_allocator, "Active Game Instances: {}", .{server.instances.items.len}) catch return;
    ...
    var y_offset: i32 = 10;
    for (server.instances.items, 0..) |instance| {
        if (y_offset >= 25) break;

        const is_wasd = instance.player.entity.ch == '@';
        const status_text = std.fmt.allocPrint(std.heap.page_allocator, "Instance {}: {} - {} - Thread: {}", .{ instance.client_id, if (is_wasd) "WASD" else "HJKL", if (instance.isRunning()) "RUNNING" else "STOPPED", if (instance.thread_handle != null) "Active" else "None" }) catch continue;
...
fn simulateNetworkInput(instance: *GameInstance, input_sequence: []const u8) void {
    std.debug.print("Simulating input for client {}\n", .{instance.client_id});
