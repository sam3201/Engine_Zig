...
const GameServer = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !GameServer {
        ...
        const server_engine = try Engine.Engine.init(
            allocator, 
            80, 
            30, 
            60, 
...
fn createVimPlayerCustom(allocator: std.mem.Allocator, start_x: i32, start_y: i32) !Player.Player {
    ...
    const entity = @import("Entity.zig").Entity.init(
        start_x, start_y, 1, 1,
        @import("Entity.zig").RenderableType.PLAYER.toId(),
        '#', cyan_color
    );
...
fn drawInstanceUI(engine: *Engine.Engine, instance: *GameInstance) void {
    ...
    const thread_info = std.fmt.allocPrint(
    ...
    const pos_text = std.fmt.allocPrint(
        std.heap.page_allocator, 
        "Position: ({}, {})", 
        .{ pos.x, pos.y }
    ) catch return;
...
fn drawServerOverview(engine: *Engine.Engine, server: *GameServer) void {
    ...
    const cpu_text = std.fmt.allocPrint(
        std.heap.page_allocator, 
        "Available CPU Cores: {}", 
        .{cpu_count}
    ) catch return;
    ...
    const instance_text = std.fmt.allocPrint(
        std.heap.page_allocator, 
        "Active Game Instances: {}", 
        .{server.instances.items.len}
    ) catch return;
    ...
    for (server.instances.items, 0..) |instance, i| {
        ...
        const status_text = std.fmt.allocPrint(
            ...
            .{ 
                instance.client_id,
                if (is_wasd) "WASD" else "HJKL",
                if (instance.isRunning()) "RUNNING" else "STOPPED",
                if (instance.thread_handle != null) "Active" else "None"
            }
        ) catch continue;
...
pub fn main() !void {
    ...
    const input_thread1 = try Thread.spawn(.{}, simulateNetworkInput, .{ 
        instance1, 
        "wwwwssssaaaadddwwww" 
    });
    
    const input_thread2 = try Thread.spawn(.{}, simulateNetworkInput, .{ 
        instance2, 
        "kkkkjjjjhhhhllllkkkk" 
    });
