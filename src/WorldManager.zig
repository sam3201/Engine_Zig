...
pub const WorldManager = struct {
    ...
    fn biomeForCoord(coord: Chunk.ChunkCoord, seed: u64) Chunk.BiomeType {
        ...
        const biome_count = Chunk.BiomeCount;
        const r = rng.random().intRangeLessThan(u32, 0, biome_count);
        ...
        while (y <= player_chunk.y + self.loaded_radius) : (y += 1) {
            ...
            while (x <= player_chunk.x + self.loaded_radius) : (x += 1) {
                ...
                if (!self.chunks.contains(coord)) {
                    const player_level: i32 = @intCast(self.player.level);
                    const biome = biomeForCoord(coord, self.player.level); // pick one for new chunk
...
pub fn randomBiome() Chunk.BiomeType {
    ...
    const roll = rand_val.intRangeLessThan(u32, 0, Chunk.BiomeCount);
