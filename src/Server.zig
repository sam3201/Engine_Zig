...
pub const GameServer = struct {
    ...
    fn handleClient(self: *GameServer) !void {
        ...
        g_reader = g_connection.stream.reader(&read_buffer);
        g_writer = g_connection.stream.writer(&write_buffer).interface;
        ...
        while (true) {
            const line = reader.readUntilDelimiterOrEof(&read_buffer, '\n') catch |err|
                {
